...
{clearTimeout(tInt);}
sendRequest("html");}
$(document).triggerHandler('ajaxProceeded');},complete:function(){$('form.cpage .cnav input[type="button"]').prop('disabled',false);$('form.cpage input[name="ccusr_tstrt"]').val(Date.now());},done:function(){$('form.cpage input[name="ccusr_rt"]').val((Date.now()-$('form.cpage input[name="ccusr_nd"]').val()));},error:function(xhr,status,errorThrown){console.log("Error: "+errorThrown);console.log("Status: "+status);console.dir(xhr);document.write(xhr.responseText);}});}}
...
{var tvlId=setTimeout(sendRequest,($('form.cpage input[name="ccfg_tvl"]').val()*1000));}}});$(document).on("ready",function(){$('form.cpage input[name="ccusr_tsrt"]').val(Date.now());if(typeof tInt!=='undefined')
{clearTimeout(tInt);}
...
$('form.cpage .cnav input[type="button"]:not(.inactive)').click(function(){if($(this).hasClass('rst'))
{$('form.cpage').trigger('reset');}
else
{$('form.cpage .cnav input[type="button"]').prop('disabled',true);if((typeof request==='undefined')||(request.readyState===0)||(request.readyState>4))
